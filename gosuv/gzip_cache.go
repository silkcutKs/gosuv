...
type ExpireHanlder struct {
	f http.Handler
}

const TimeFormat = "Mon, 02 Jan 2006 15:04:05 GMT"

func NewExpireHanlder(f http.Handler) *ExpireHanlder {
	return &ExpireHanlder{
		f: f,
	}
}
func (l*ExpireHanlder) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	l.f.ServeHTTP(w, r)

	w.Header().Set("Expires", time.Now().AddDate(1, 1, 0).UTC().Format(TimeFormat))
